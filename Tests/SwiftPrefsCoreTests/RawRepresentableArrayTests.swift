...
struct RawRepresentableArrayTests {
    ...
    func intRepresentable(schema: TestSchema) throws {
        ...
        // check underlying storage is as expected
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.arrayB.rawValue) as? [Int])
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.arrayBDefaulted.rawValue) as? [Int])
    }
}
