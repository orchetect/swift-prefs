...
struct CodableDictionaryTests {
    ...
    func singleJSONString(schema: TestSchema) throws {
        ...
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictA.rawValue) as? String)
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictADefaulted.rawValue) as? String)
    }
    ...
    func singleJSONData(schema: TestSchema) throws {
        ...
        // check underlying storage is as expected
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictB.rawValue) as? Data)
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictBDefaulted.rawValue) as? Data)
    }
    ...
    func arrayOfJSONStringElements(schema: TestSchema) throws {
        ...
        // check underlying storage is as expected
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictC.rawValue) as? [String: String])
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictCDefaulted.rawValue) as? [String: String])
    }
    ...
    func arrayOfJSONDataElements(schema: TestSchema) throws {
        ...
        // check underlying storage is as expected
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictD.rawValue) as? [String: Data])
        _ = try #require(schema.storage.unsafeStorageValue(forKey: TestSchema.Key.dictDDefaulted.rawValue) as? [String: Data])
    }

    /// Edge case test to ensure we don't run afoul of force-unwrap exceptions.
    @available(macOS 14.0, iOS 17.0, watchOS 10.0, tvOS 17.0, *)
    @Test(arguments: schemas)
    func modifyNonExistentKey(schema: TestSchema) {
