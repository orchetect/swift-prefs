...
struct ChainingEncodingStrategiesTests {
    enum MyType: Int {
        case foo = 0x1234_5678
        case bar = 0x9876_5432
    }
    ...
    final class TestSchema: @unchecked Sendable {
        ...
        @Pref(coding: .compressedData(algorithm: .lzfse).base64DataString())
        var foo: Data?
        ...
        var fpsA: FloatingPointSign?

        @Pref(coding: FloatingPointSign.rawRepresentablePrefsCoding.intAsString)
        var fpsB: FloatingPointSign?

        @Pref(coding: .uInt32AsInt)
        var binaryInt: UInt32?
    }

    @available(macOS 14.0, iOS 17.0, watchOS 10.0, tvOS 17.0, *)
    @Test
    func chainedEncodingA() throws {
