...
struct CustomEncodingTests {
    ...
    final class TestSchema: @unchecked Sendable {
        ...
        @Pref(key: Key.custom, coding: CustomPrefCoding())
        var custom: NonCodableNonRawRepresentable?
        @Pref(key: Key.customDefaulted, coding: CustomPrefCoding())
        var customDefaulted: NonCodableNonRawRepresentable = .init(value: 1)
    }
